...
export const MyRFQs: React.FC = () => {
  ...
  const buyerName = currentUser?.displayName || 'थोक वस्त्र वितरक';
  ...
  const handleSubmitRFQ = async (e: React.FormEvent) => {
    ...
    if (!productDescription.trim() || !requiredQuantity || !deadline) {
      setFormError('कृपया सभी आवश्यक फ़ील्ड भरें (Please fill out all required fields)');
      return;
    }
    ...
    try {
      ...
      setFormSuccess('थोक मांग अनुरोध (RFQ) सफलतापूर्वक जमा किया गया! (RFQ Submitted!)');
      ...
      setFormError('आरएफक्यू सबमिट करने में त्रुटि। कृपया पुनः प्रयास करें।');
    } finally {
  ...
  const handleAcceptQuote = async (rfqId: string, coalitionId: string) => {
    if (window.confirm('क्या आप गठबंधन के इस एकीकृत कोटेशन को स्वीकार करना चाहते हैं? (Accept this quotation?)')) {
      try {
        await updateRFQStatus(rfqId, 'accepted');
        await updateCoalitionStatus(coalitionId, 'accepted');
        alert('कोटेशन स्वीकार किया गया! सहकारी समितियों को अनुबंध आवंटन भेज दिया गया है।');
        fetchData();
      } catch (err) {
        console.error("Error accepting quote:", err);
        alert('स्वीकृति प्रसंस्करण में त्रुटि।');
      }
    }
  };

  const handleDeclineQuote = async (rfqId: string, coalitionId: string) => {
    if (window.confirm('क्या आप गठबंधन के इस कोटेशन को अस्वीकार करना चाहते हैं?')) {
      try {
        await updateRFQStatus(rfqId, 'declined');
        await updateCoalitionStatus(coalitionId, 'declined');
        alert('कोटेशन अस्वीकार कर दिया गया है।');
        fetchData();
      } catch (err) {
        console.error("Error declining quote:", err);
        alert('अस्वीकृति प्रसंस्करण में त्रुटि।');
      }
    }
  };
  ...
  const getStatusHindi = (status: BuyerRFQ['status']) => {
    switch (status) {
      case 'open':
    ...
    <BuyerLayout>
      ...
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="font-heading text-3xl md:text-4xl font-bold text-loom-wood flex items-center gap-2">
            ...
            थोक कोटेशन मांग बोर्ड (My RFQs & Quote Board)
          </h1>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="font-heading text-lg text-loom-wood">आरएफक्यू डैशबोर्ड लोड हो रहा है...</p>
        </div>
      ) : rfqs.length === 0 ? (
        <div className="text-center py-16 bg-white/60 rounded-2xl border-2 border-dashed border-loom-beige p-8">
          ...
          <h3 className="font-heading text-2xl font-bold text-loom-wood mb-2">कोई थोक मांग अनुरोध जमा नहीं है</h3>
            ...
            return (
              <Card key={rfq.rfqId} className="vintage-card overflow-hidden border-2 border-loom-beige">
                <CardHeader className="bg-loom-parchment p-5 border-b border-loom-beige flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                  ...
                  <div className="flex gap-4 text-xs font-bold text-loom-ink-light">
                    <div className="flex items-center gap-1.5">
                      ...
                      अंतिम तिथि: {new Date(rfq.deadline).toLocaleDateString('hi-IN')}
                    </div>
                    <div className="flex items-center gap-1.5">
                      <TrendingUp className="w-4 h-4 text-loom-gold" />
                      लक्षित बजट: {rfq.maxBudgetPerUnit ? `₹${rfq.maxBudgetPerUnit}/पीस` : 'संवाद योग्य'}
                    </div>
                  </div>
                </CardHeader>
                ...
                <CardContent className="p-5 space-y-4">
                  ...
                  <div>
                    <span className="text-xs uppercase tracking-wider font-extrabold text-loom-wood block mb-1">वांछित उत्पाद विवरण (Required Specifications):</span>
                    ...
                    <div className="mt-4 border border-purple-200 bg-purple-50/20 rounded-2xl p-4 space-y-3">
                      <div className="flex items-center justify-between border-b border-purple-100 pb-2">
                        <span className="text-sm font-extrabold text-purple-950 flex items-center gap-1.5">
                          ...
                          सहकारी गठबंधन की एकीकृत पेशकश (Collective Coalition Proposal):
                        </span>
                      ...
                      <div className="bg-purple-100/40 p-3 rounded-xl border border-purple-200 flex justify-between items-center text-sm">
                        <span className="font-extrabold text-purple-950">कुल एकीकृत उद्धरण मूल्य (Total Collective Price):</span>
                        <span className="font-black text-lg text-loom-wood">
                          ₹{coalition.totalQuotePrice.toLocaleString('hi-IN')}
                        </span>
                      </div>
                      ...
                      {rfq.status === 'quote_submitted' && (
                        <div className="flex justify-end gap-3 pt-3">
                          ...
                          >
                            ...
                            कोटेशन स्वीकार करें (Accept Proposal)
                          </Button>
                        </div>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-loom-cream border-4 border-loom-gold w-full max-w-lg rounded-2xl shadow-2xl relative flex flex-col">
            <div className="p-5 border-b border-loom-beige bg-loom-wood text-loom-cream rounded-t-lg bg-loom-pattern relative">
              ...
              <div className="relative z-10">
                <h3 className="font-heading text-xl font-bold flex items-center gap-1.5">
                  ...
                  नया थोक मांग अनुरोध (Post Bulk RFQ)
                </h3>
            ...
            <form onSubmit={handleSubmitRFQ} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-loom-wood mb-1">उत्पाद विवरण एवं विशेषताएं (Specifications) *</label>
                <textarea
                  ...
                  placeholder="वांछित वस्त्र, धागे की किस्म, बुनाई तकनीक (उदा. जरी बॉर्डर, बनारसी सूती कपड़ा), पैटर्न और गुणवत्ता मानक स्पष्ट रूप से लिखें..."
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-loom-wood mb-1">आवश्यक मात्रा (Quantity in Pieces) *</label>
                  <Input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-bold text-loom-wood mb-1">अधिकतम दर प्रति यूनिट (Target Price/Piece ₹)</label>
                  <Input
                    type="number"
              ...
              <div>
                <label className="block text-xs font-bold text-loom-wood mb-1">आपूर्ति की अंतिम तिथि (Required Deadline) *</label>
                <Input
                  type="date"
                  value={deadline}
              ...
              <div className="flex justify-end gap-2.5 pt-4 border-t border-loom-beige">
                ...
                >
                  {formSubmitting ? 'प्रक्रिया चालू...' : 'मांग अनुरोध प्रकाशित करें'}
                </Button>
              </div>
            </form>
          </div>
