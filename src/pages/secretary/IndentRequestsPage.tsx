...
export const IndentRequestsPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error("डेटा लोड करने में त्रुटि हुई", { className: 'vintage-toast' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSelectRequest = (req: IndentRequest) => {
    if (selectedIds.includes(req.requestId)) {
      ...
      if (selectedMaterialName && selectedMaterialName !== req.materialName) {
        toast.error("केवल एक ही प्रकार की सामग्री का समेकन किया जा सकता है", { className: 'vintage-toast' });
        return;
      }
  ...
  const handleConsolidate = async () => {
    ...
    try {
      ...
      toast.success("बल्क इंडेंट सफलतापूर्वक बनाया गया!", { className: 'vintage-toast' });
      ...
      toast.error("समेकन प्रक्रिया विफल रही", { className: 'vintage-toast' });
    }
  };
  ...
  const handleVendorSelect = async (vendor: Vendor, quote?: number) => {
    ...
    try {
      ...
      toast.success(`सामग्री मांग पत्र ${vendor.name} को भेजा गया!`, { className: 'vintage-toast' });
      ...
      toast.error("विक्रेता को भेजने में त्रुटि", { className: 'vintage-toast' });
    }
  };
  ...
  const handleMarkAsOrdered = async (indent: BulkIndent) => {
    try {
      ...
      toast.success("सफलतापूर्वक 'ऑर्डर किया गया' चिह्नित किया गया", { className: 'vintage-toast' });
      await loadData();
    } catch (err) {
      console.error("Error transition to ordered:", err);
      toast.error("ऑर्डर चिह्नित करने में असमर्थ", { className: 'vintage-toast' });
    }
  };
  ...
  const handleMarkAsReceived = async (indent: BulkIndent) => {
    try {
      ...
      toast.success("सामग्री प्राप्त हुई! स्टॉक मात्रा बढ़ाई गई है।", { className: 'vintage-toast' });
      await loadData();
    } catch (err) {
      console.error("Error transition to received:", err);
      toast.error("सामग्री प्राप्त चिह्नित करने में असमर्थ", { className: 'vintage-toast' });
    }
  };
  ...
  const handleSubmitQuote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("कोटेशन मूल्य सहेज लिया गया है।", { className: 'vintage-toast' });
      setQuoteModalOpen(false);
      setQuotingIndent(null);
      await loadData();
    } catch (err) {
      console.error("Error updating quote:", err);
      toast.error("कोटेशन मूल्य अद्यतन करने में विफल", { className: 'vintage-toast' });
    }
  };
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="bg-white/40 p-5 rounded-2xl border border-loom-beige/50 backdrop-blur-xs">
          <h1 className="font-heading text-3xl font-bold text-loom-wood flex items-center gap-2">
            ...
            सामूहिक खरीद एवं इंडेंट प्रबंधन (Bulk Procurement & Indent)
          </h1>
        ...
        <div className="flex border-b border-loom-beige gap-2">
          ...
          >
            लंबित अनुरोध ({pendingRequests.length})
          </button>
          <button
          ...
          >
            समेकित बल्क इंडेंट ({bulkIndents.length})
          </button>
        </div>
        ...
        {loading ? (
          <div className="py-24 flex flex-col items-center justify-center gap-2">
            ...
            <p className="font-heading text-loom-wood mt-4 text-sm animate-pulse">लोड हो रहा है...</p>
          </div>
        ) : activeTab === 'pending' ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center bg-white p-4 rounded-xl border border-loom-beige shadow-xs">
              <input
                type="text"
                placeholder="बुनकर का नाम या सामग्री खोजें..."
              ...
              {selectedIds.length > 0 && (
                <div className="flex items-center gap-3 sm:ml-auto bg-loom-gold/10 p-2 px-4 rounded-xl border border-loom-gold/30">
                  <span className="text-xs font-body text-loom-wood font-semibold">
                    चयनित: <strong className="font-bold text-sm">{selectedIds.length}</strong> अनुरोध ({selectedMaterialName})
                  </span>
                  <button
            ...
            {filteredPending.length === 0 ? (
              <div className="vintage-card p-12 text-center max-w-2xl mx-auto">
                ...
                <p className="font-heading font-semibold text-loom-wood">कोई लंबित अनुरोध नहीं है</p>
                  ...
                  <table className="w-full border-collapse text-left font-body text-sm">
                    <thead>
                      <tr className="bg-loom-wood text-white font-heading">
                        <th className="p-4 w-12 text-center">चयन</th>
                        <th className="p-4">बुनकर का नाम</th>
                        <th className="p-4">कच्चा माल</th>
                        <th className="p-4 text-right">मात्रा</th>
                        <th className="p-4">आवश्यकता तिथि</th>
                        <th className="p-4">अनुरोध दिनांक</th>
                      </tr>
                    </thead>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-4 text-loom-ink-light text-xs">
                              {req.requiredByDate ? new Date(req.requiredByDate).toLocaleDateString('hi-IN') : 'शीघ्र ही'}
                            </td>
                            <td className="p-4 text-loom-ink-light text-xs">
          ...
          <div className="space-y-4">
            {bulkIndents.length === 0 ? (
              <div className="vintage-card p-12 text-center max-w-2xl mx-auto">
                <span className="text-5xl block mb-2">📝</span>
                <p className="font-heading font-semibold text-loom-wood">कोई बल्क इंडेंट उपलब्ध नहीं है</p>
                <p className="font-body text-xs text-loom-ink-light mt-1">
                      ...
                      <div className="space-y-2 flex-1">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="text-xs text-loom-ink-light flex items-center gap-1">
                            <Calendar className="w-3.5 h-3.5 text-loom-gold" /> {new Date(indent.createdAt).toLocaleDateString('hi-IN')}
                          </span>
                        </div>

                        <div className="flex items-baseline gap-2">
                          <h3 className="font-heading text-lg font-bold text-loom-wood">{indent.materialName}</h3>
                          <span className="font-body text-sm text-loom-ink-light">कुल मात्रा:</span>
                          <span className="font-mono font-bold text-base text-loom-ink bg-loom-sand/15 px-2.5 py-0.5 rounded-md border border-loom-beige/50">
                            {indent.totalQuantity} {indent.unit}
                          </span>
                        </div>
                        ...
                        {indent.vendorName ? (
                          <div className="p-3 bg-loom-cream/40 rounded-xl border border-loom-beige/40 inline-flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
                            <span className="font-body text-loom-ink flex items-center gap-1">
                              ...
                              <strong className="text-loom-wood">विक्रेता:</strong> {indent.vendorName}
                            </span>
                            {indent.vendorQuote !== undefined && (
                              <span className="font-body text-loom-ink flex items-center gap-1">
                                <DollarSign className="w-3.5 h-3.5 text-loom-gold" />
                                <strong className="text-loom-wood">कोटेशन:</strong> ₹{indent.vendorQuote.toLocaleString('en-IN')}
                              </span>
                            )}
                          </div>
                        ) : (
                          <p className="text-xs font-body text-amber-600 italic">⚠️ कोई विक्रेता असाइन नहीं किया गया है</p>
                        )}

                        <div className="text-xs font-body text-loom-ink-light">
                          समेकित बुनकर अनुरोधों की संख्या: <strong className="text-loom-wood">{indent.requestIds.length}</strong>
                        </div>
                      </div>
                        ...
                        <div>
                          {indent.status === 'draft' && (
                            <span className="px-3 py-1 text-xs rounded-full bg-slate-100 text-slate-700 border border-slate-300 font-bold flex items-center gap-1">
                              <Clock className="w-3.5 h-3.5" /> ड्राफ्ट (Draft)
                            </span>
                          )}
                          {indent.status === 'sent_to_vendor' && (
                            <span className="px-3 py-1 text-xs rounded-full bg-blue-50 text-blue-700 border border-blue-200 font-bold flex items-center gap-1">
                              <Truck className="w-3.5 h-3.5 animate-pulse" /> विक्रेता को भेजा गया
                            </span>
                          )}
                          {indent.status === 'ordered' && (
                            <span className="px-3 py-1 text-xs rounded-full bg-amber-50 text-amber-700 border border-amber-200 font-bold flex items-center gap-1">
                              <CheckCircle2 className="w-3.5 h-3.5" /> आर्डर किया गया
                            </span>
                          )}
                          {indent.status === 'received' && (
                            <span className="px-3 py-1 text-xs rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold flex items-center gap-1">
                              <CheckCircle2 className="w-3.5 h-3.5" /> सामग्री प्राप्त हुई
                            </span>
                          )}
                        </div>
            ...
            <div className="vintage-card w-full max-w-sm bg-loom-parchment overflow-hidden border-t-8 border-loom-gold relative">
              <form onSubmit={handleSubmitQuote}>
                <div className="p-5 border-b border-loom-beige bg-loom-sand/10 flex justify-between items-center">
                  <h3 className="font-heading text-lg font-bold text-loom-wood">कोटेशन मूल्य दर्ज करें</h3>
                  <button
                    type="button"
                    onClick={() => {
                      setQuoteModalOpen(false);
                ...
                <div className="p-5 space-y-4">
                  <div className="space-y-1">
                    <label className="block text-xs font-heading font-bold text-loom-wood">
                      कुल कोटेशन मूल्य (₹) <span className="text-loom-error">*</span>
                    </label>
