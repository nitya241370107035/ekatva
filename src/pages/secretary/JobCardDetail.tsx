...
export const JobCardDetail: React.FC = () => {
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      if (card) {
        ...
        setToastMessage('कार्य कार्ड नहीं मिला।');
      }
    } catch (err) {
  ...
  const handleQcPassAndCreateInstance = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createPayment({
        ...
        type: 'wage',
        amount: totalWage,
        description: `मजदूरी – ${jobCard.title} (मात्रा: ${jobCard.quantity} पीस, प्रति पीस दर: ₹${jobCard.wagePerPiece})`,
        jobCardId: jobCardId,
      ...
      const updatedLogList = [
        ...statusLog,
        {
          ...
          remarks: 'गुणवत्ता नियंत्रण पास।'
        }
      ];

      const rawSteps = updatedLogList.map(log => ({
        step: log.status === 'assigned' ? 'धागा जारी और कार्य आवंटित (Yarn Issued)' :
              log.status === 'in_progress' ? 'बुनाई शुरू (Weaving Started)' :
              log.status === 'completed' ? 'बुनाई पूर्ण (Weaving Completed)' :
              log.status === 'qc_passed' ? 'गुणवत्ता स्वीकृत (QC Passed)' : log.status,
        timestamp: typeof log.timestamp === 'string' ? log.timestamp : new Date().toISOString(),
        details: log.remarks || `स्थिति ${log.status} में बदली गई`
      }));
      ...
      setToastMessage('गुणवत्ता नियंत्रण (QC) स्वीकृत हुआ, मजदूरी भुगतान दर्ज किया गया और डिजिटल ट्रेसेबिलिटी प्रमाणपत्र (QR) तैयार हो गया!');
      setQcPassModalOpen(false);
      await fetchDetails();
    } catch (err) {
      console.error("Error passing QC & creating traceability:", err);
      setToastMessage('QC और ट्रेसेबिलिटी प्रमाणपत्र बनाने में त्रुटि आई।');
    } finally {
  ...
  const handleQcRejectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToastMessage('QC अस्वीकार कर दिया गया। त्रुटि बुनकर को भेज दी गई है और विश्वसनीयता अंक घटा दिए गए हैं।');
      ...
      setToastMessage('QC सबमिट करने में त्रुटि आई।');
    } finally {
      setQcActionLoading(false);
    }
  };
      ...
      <html>
        ...
        <body>
          <div class="label-card">
            ...
            <div class="weaver-info">बुनकर: ${productInstance.weaverName}</div>
            <div class="fair-wage-badge">सत्यापित निष्पक्ष वेतन (${productInstance.wagePercentage}%)</div>
  ...
  const getStatusLabelAndIcon = (status: JobCard['status']) => {
    switch (status) {
      case 'assigned':
        return { label: 'असाइन किया गया', icon: Clock, color: 'text-blue-600 bg-blue-50 border-blue-200' };
      case 'in_progress':
        return { label: 'कार्य प्रगति पर', icon: Hammer, color: 'text-amber-600 bg-amber-50 border-amber-200' };
      case 'completed':
        return { label: 'कार्य पूर्ण (QC हेतु लंबित)', icon: ClipboardCheck, color: 'text-green-600 bg-green-50 border-green-200' };
      case 'qc_passed':
        return { label: 'गुणवत्ता स्वीकृत (QC PASS)', icon: FileCheck2, color: 'text-emerald-700 bg-emerald-50 border-emerald-300' };
      case 'qc_rejected':
        return { label: 'गुणवत्ता अस्वीकृत (QC FAIL)', icon: AlertOctagon, color: 'text-red-700 bg-red-50 border-red-300' };
    }
  };
    ...
    return (
      <SecretaryLayout>
        <div className="py-24 flex flex-col items-center justify-center gap-2">
          ...
          <p className="font-heading text-loom-wood mt-2 animate-pulse font-semibold">कार्य विवरण प्राप्त किया जा रहा है...</p>
        </div>
      </SecretaryLayout>
    );
  }
    ...
    return (
      <SecretaryLayout>
        <div className="text-center py-24">
          ...
          <h2 className="font-heading text-2xl font-bold text-loom-wood">कार्य कार्ड नहीं मिला</h2>
          <button
            onClick={() => navigate('/secretary/production')}
            className="vintage-button mt-4 px-6 py-2"
          >
            उत्पादन बोर्ड पर लौटें
          </button>
        </div>
      </SecretaryLayout>
    );
          ...
          <div className="vintage-card p-6 sm:p-8 relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
              ...
              <div className="p-3 bg-loom-sand/20 border-2 border-loom-beige rounded-xl flex items-center gap-2 text-loom-wood shrink-0">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-heading font-bold uppercase tracking-wider text-loom-ink/50 leading-none">अंतिम तिथि</span>
                  <span className="font-body font-bold text-sm leading-tight mt-0.5">
                    {new Date(jobCard.deadline).toLocaleDateString('hi-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 py-6 border-y border-loom-beige/50">
              <div className="space-y-0.5">
                <span className="text-xs font-heading font-bold text-loom-ink/50 uppercase">सौंपा गया बुनकर</span>
              ...
              <div className="space-y-0.5">
                <span className="text-xs font-heading font-bold text-loom-ink/50 uppercase">उत्पाद मात्रा</span>
                <p className="font-body text-xl font-bold text-loom-ink">
                  {jobCard.quantity} पीस (Pieces)
                </p>
              </div>

              <div className="space-y-0.5 col-span-2 sm:col-span-1">
                <span className="text-xs font-heading font-bold text-loom-ink/50 uppercase">मज़दूरी दर / पीस</span>
                <p className="font-body text-xl font-bold text-loom-ink flex items-center">
                  <IndianRupee className="w-4.5 h-4.5 text-loom-gold" />
                  {jobCard.wagePerPiece} / पीस
                </p>
              </div>
            </div>
            ...
            <div className="mt-6">
              <h3 className="font-heading text-lg font-bold text-loom-wood mb-3 flex items-center gap-1.5">
                ...
                जारी की गई कच्ची सामग्री (Raw Materials Issued)
              </h3>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-loom-sand/40 border-b border-loom-beige/50 text-xs font-heading font-bold text-loom-wood">
                      <th className="p-3">सामग्री का नाम (Material)</th>
                      <th className="p-3 text-right">मात्रा (Quantity)</th>
                  ...
                  <tbody>
                    {jobCard.rawMaterialsIssued.map((item, idx) => (
                      <tr key={idx} className="border-b border-loom-beige/30 last:border-b-0 font-body text-sm text-loom-ink">
                        <td className="p-3 font-semibold">{item.materialName}</td>
                        <td className="p-3 text-right font-bold">{item.quantity}</td>
                        <td className="p-3 text-loom-ink/70">{item.unit}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {jobCard.status === 'qc_passed' && !productInstance && (
              <div className="mt-6 p-4 bg-emerald-50 border-2 border-emerald-200 rounded-xl flex items-center justify-between">
                <div>
                  <h4 className="font-heading font-bold text-emerald-800">कुल देय मजदूरी (Total Earned Wages)</h4>
                  <p className="font-body text-xs text-emerald-700">यह मजदूरी गुणवत्ता स्वीकृत होने के बाद बुनकर के खाते में स्वीकृत की गई है।</p>
                </div>
            ...
            {jobCard.status === 'qc_passed' && productInstance && (
              <div className="mt-6 p-6 bg-[#fdfaf2] border-2 border-[#d4af37] rounded-xl relative overflow-hidden">
                ...
                <h4 className="font-heading text-base font-bold text-[#854d0e] mb-4 flex items-center gap-1.5">
                  ...
                  हैंडलूम सत्यता एवं निष्पक्ष मजदूरी प्रमाणपत्र
                </h4>
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 space-y-3 w-full">
                    <div className="grid grid-cols-2 gap-4 pb-2 border-b border-loom-beige/50">
                      <div>
                        <span className="text-[11px] font-heading font-semibold text-loom-ink/60">उत्पाद का नाम</span>
                        <p className="font-heading font-bold text-loom-wood text-sm leading-tight">{productInstance.productName}</p>
                      </div>
                      <div>
                        <span className="text-[11px] font-heading font-semibold text-loom-ink/60">बुनकर का नाम</span>
                    ...
                    <div className="grid grid-cols-3 gap-2 py-1 text-center">
                      <div className="bg-emerald-50 border border-emerald-100 p-1.5 rounded-lg">
                        <span className="text-[9px] font-heading font-bold text-emerald-850 uppercase block">बुनकर मजदूरी</span>
                      ...
                      <div className="bg-amber-50 border border-amber-100 p-1.5 rounded-lg">
                        <span className="text-[9px] font-heading font-bold text-amber-855 uppercase block">रिटेल मूल्य</span>
                      ...
                      <div className="bg-purple-50 border border-purple-100 p-1.5 rounded-lg">
                        <span className="text-[9px] font-heading font-bold text-purple-855 uppercase block">मजदूरी हिस्सा</span>
                    ...
                    <div className="p-2.5 bg-emerald-50/50 border border-emerald-200/50 rounded-xl text-[11px] text-emerald-800 flex gap-2 items-start mt-1">
                      ...
                      <div>
                        <span className="font-semibold block">सत्यापित निष्पक्ष मजदूरी (Certified Fair Wage):</span>
                        इस वस्त्र के कुल मूल्य का <strong>{productInstance.wagePercentage}%</strong> सीधा बुनकर के बैंक खाते में गया है।
                      </div>
                    </div>

                    <div className="flex items-center justify-between pt-1">
                      <span className="text-[9px] font-mono text-loom-ink/40">प्रमाणपत्र आईडी: <span className="font-bold">{productInstance.instanceId.slice(0, 8)}...</span></span>
                      <a
            ...
            {jobCard.status === 'qc_rejected' && (
              <div className="mt-6 p-4 bg-red-50 border-2 border-red-200 rounded-xl space-y-3">
                <div>
                  <h4 className="font-heading font-bold text-red-800">गुणवत्ता अस्वीकरण का विवरण (Rejection Details)</h4>
                  <p className="font-body text-sm text-red-700 mt-1">
                    <strong>कारण:</strong> {jobCard.qcRemarks || 'बिना टिप्पणी के अस्वीकृत।'}
                  </p>
                </div>
                {jobCard.qcPhotoURL && (
                  <div className="mt-2">
                    <span className="block text-xs font-heading font-bold text-red-800 mb-1">त्रुटि चित्र (Error Photo):</span>
                    <img 
            ...
            <div className="vintage-card p-6 border-t-4 border-loom-gold bg-loom-cream flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h2 className="font-heading text-xl font-bold text-loom-wood flex items-center gap-1.5">
                  ...
                  गुणवत्ता नियंत्रण जांच (Quality Control Check)
                </h2>
                  ...
                  <div key={log.logId || idx} className="relative">
                    ...
                    <div>
                      ...
                      <h4 className="font-heading font-bold text-loom-wood text-base capitalize">
                        {log.status === 'assigned' && 'असाइन किया गया'}
                        {log.status === 'in_progress' && 'बुनाई शुरू'}
                        {log.status === 'completed' && 'कार्य पूर्ण किया गया'}
                        {log.status === 'qc_passed' && 'QC पास (मंजूर)'}
                        {log.status === 'qc_rejected' && 'QC असफल (अस्वीकृत)'}
                      </h4>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-loom-cream border-4 border-loom-gold rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden p-6 relative">
            <h3 className="font-heading text-2xl font-bold text-loom-wood mb-4 pb-2 border-b-2 border-loom-beige">
              गुणवत्ता विफलता कारण और फोटो दर्ज करें
            </h3>
            ...
            <form onSubmit={handleQcRejectSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-heading font-bold text-loom-wood mb-1.5">
                  अस्वीकृति का स्पष्ट कारण (Reason for Rejection) *
                </label>
                <textarea
                  required
                  rows={4}
                  placeholder="कृपया बुनाई में पाई गई त्रुटि या दोष का सटीक विवरण दें ताकि बुनकर इसे सुधार सके..."
                  value={rejectReason}
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-loom-beige hover:border-loom-gold rounded-xl p-4 flex flex-col items-center justify-center bg-white cursor-pointer transition-colors relative">
                  ...
                  <span className="text-xs text-loom-ink/50 mt-1 font-body">
                    बुनाई दोष दिखाने के लिए एक फाइल अपलोड करें
                  </span>
                </div>
                ...
                {imagePreview && (
                  <div className="mt-3 flex items-center gap-3 bg-loom-sand/20 p-2.5 rounded-lg border border-loom-beige">
                    ...
                    <div className="flex-1 overflow-hidden">
                      <p className="text-xs font-heading font-bold text-loom-wood truncate">चयनित फोटो</p>
              ...
              <div className="flex gap-3 justify-end pt-4 border-t border-loom-beige/50">
                ...
                >
                  {qcActionLoading ? 'सहेज रहे हैं...' : 'त्रुटि सबमिट करें'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-loom-cream border-4 border-loom-gold rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden p-6 relative animate-in fade-in zoom-in-95 duration-200">
            <h3 className="font-heading text-2xl font-bold text-loom-wood mb-2 pb-2 border-b-2 border-loom-beige flex items-center gap-1.5">
              ...
              QC पास और न्यायसंगत वेतन सेटअप
            </h3>
            ...
            <form onSubmit={handleQcPassAndCreateInstance} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-heading font-bold text-loom-wood mb-1">
                  सहकारी स्टोरफ्रंट उत्पाद से लिंक करें (Link to Storefront Product)
                </label>
                <select
                  ...
                  className="w-full px-4 py-2 border-2 border-loom-beige rounded-xl focus:border-loom-gold focus:outline-none bg-white text-loom-ink font-body"
                >
                  <option value="">-- लिंक नहीं करें (सामान्य उत्पाद) --</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-heading font-bold text-loom-wood mb-1">
                    बुनकर मजदूरी भुगतान (Earned Wage)
                  </label>
              ...
              {finalPrice > 0 && jobCard && (
                <div className="p-3 bg-purple-50 border border-purple-100 rounded-xl flex items-center justify-between text-xs text-purple-800">
                  <span className="font-heading font-bold">कारीगर हिस्सा (Weaver Wage Ratio):</span>
              ...
              <div className="flex gap-3 justify-end pt-4 border-t border-loom-beige/50">
                ...
                >
                  ...
                  {qcActionLoading ? 'सहेज रहे हैं...' : 'प्रमाणित करें और QC पास करें'}
                </button>
              </div>
            </form>
          </div>
