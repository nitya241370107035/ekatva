  ...
  const handleStatusChange = async (newStatus: 'open' | 'in_progress' | 'resolved') => {
    ...
    try {
      ...
      if (newStatus === 'in_progress') {
        statusSystemText = '🛠️ [सिस्टम सूचना]: सचिव द्वारा शिकायत की स्थिति "प्रगति पर" अद्यतन की गई है।';
      } else if (newStatus === 'resolved') {
        statusSystemText = '✅ [सिस्टम सूचना]: सचिव द्वारा शिकायत का निवारण कर इसे "हल" चिह्नित किया गया है।';
      }

      await addGrievanceMessage(id, {
        senderId: 'system',
        senderName: 'समिति प्रणाली',
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      await addGrievanceMessage(id, {
        ...
        senderName: userProfile.displayName || 'सचिव साहब',
        senderRole: 'secretary',
        text: replyText.trim()
      });
      setReplyText('');
    ...
    return (
      <SecretaryLayout>
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          <p className="font-heading text-2xl text-loom-wood animate-pulse">शिकायत विवरणी लोड हो रही है...</p>
        </div>
      </SecretaryLayout>
    );
  }
    ...
    return (
      <SecretaryLayout>
        <div className="p-8 text-center vintage-card">
          ...
          <h2 className="font-heading text-2xl font-bold text-loom-wood">शिकायत टिकट नहीं मिला</h2>
          <button 
          ...
          >
            <ArrowLeft className="w-4 h-4" /> शिकायत सूची में वापस जाएं
          </button>
        </div>
      </SecretaryLayout>
    );
  ...
  return (
    <SecretaryLayout>
      ...
      <div className="mb-6">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> शिकायत निवारण पटल पर वापस जाएं (Back to List)
        </button>
        <h1 className="font-heading text-3xl font-bold text-loom-wood">
          शिकायत समाधान संवाद (Resolve Grievance)
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="space-y-6">
          <div className="vintage-card p-6 bg-loom-cream border-t-4 border-t-loom-wood shadow-md">
            <span className="text-xs text-loom-gold font-bold font-heading uppercase tracking-widest block mb-2">शिकायत विवरण (Grievance Details)</span>
            ...
            <div className="border-t border-b border-loom-beige/30 py-3.5 my-4 space-y-2.5 font-body text-base text-loom-ink">
              <div>
                <span className="text-xs text-loom-ink-light font-bold block">शिकायतकर्ता (Weaver):</span>
                <span className="font-semibold text-loom-wood">{grievance.weaverName}</span>
              </div>
              <div>
                <span className="text-xs text-loom-ink-light font-bold block">शिकायत श्रेणी (Category):</span>
                <span className="font-semibold">{getCategoryText(grievance.category)}</span>
              </div>
              <div>
                <span className="text-xs text-loom-ink-light font-bold block">दर्ज करने की तिथि (Date Created):</span>
                <span className="font-mono text-sm">{new Date(grievance.createdAt).toLocaleDateString('hi-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
              </div>
              <div className="flex items-center gap-2 pt-1.5">
                <span className="text-xs text-loom-ink-light font-bold">स्थिति (Status):</span>
            ...
            <div>
              <span className="text-xs text-loom-ink-light font-bold font-heading block mb-2.5 uppercase tracking-wide">शिकायत की स्थिति बदलें (Update Status)</span>
              <div className="flex flex-col gap-2">
                {grievance.status !== 'in_progress' && grievance.status !== 'resolved' && (
                  ...
                  >
                    ...
                    प्रगति पर चिह्नित करें (Mark In Progress)
                  </Button>
                )}
                {grievance.status !== 'resolved' && (
                  ...
                  >
                    ...
                    निवारण पूर्ण / हल करें (Mark Resolved)
                  </Button>
                )}
                {grievance.status === 'resolved' && (
                  <Button
                  ...
                  >
                    ...
                    शिकायत पुनः खोलें (Re-open Grievance)
                  </Button>
                )}
              </div>
            </div>
        ...
        <div className="lg:col-span-2">
          <div className="vintage-card p-6 bg-loom-cream border-t-4 border-t-loom-gold shadow-md flex flex-col h-[580px]">
            <h3 className="font-heading text-lg font-bold text-loom-wood flex items-center gap-2 border-b border-loom-beige/30 pb-3 mb-4">
              ...
              बुनकर के साथ संवाद सूत्र (Conversation Thread)
            </h3>
            ...
            {grievance.status === 'resolved' ? (
              ...
              <form onSubmit={handleSendReply} className="flex gap-2 pt-2 border-t border-loom-beige/30">
                <textarea
                  placeholder="अपना उत्तर लिखें... (Type your reply...)"
                  value={replyText}
