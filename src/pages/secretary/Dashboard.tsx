    ...
    const fetchDashboardStats = async () => {
      try {
        ...
        if (weavers.length > 0) {
          ...
          tempActivities.push({
            text: `नया बुनकर पंजीकरण: ${sortedWeavers[0].displayName}`,
            time: 'हालिया',
            tag: 'बुनकर'
          });
        }
        if (notices.length > 0) {
          tempActivities.push({
            text: `नयी सूचना प्रकाशित: "${notices[0].title}"`,
            time: 'हालिया',
            tag: 'सूचना'
          });
        }
        if (meetings.length > 0) {
          tempActivities.push({
            text: `बैठक दर्ज की गई: "${meetings[0].title}"`,
            time: 'हालिया',
            tag: 'बैठक'
          });
        }
        if (grievances.length > 0) {
          tempActivities.push({
            text: `शिकायत शिकायतकर्ता: ${grievances[0].weaverName} - "${grievances[0].subject}"`,
            time: 'हालिया',
            tag: 'शिकायत'
          });
        }
        ...
        if (tempActivities.length === 0) {
          tempActivities.push({
            text: 'समिति का डिजिटल क्रेडेंशियल सृजन',
            time: 'गठन सत्र',
            tag: 'प्रणाली'
          });
        }
    ...
    <SecretaryLayout>
      ...
      <div className="mb-8 p-6 bg-loom-cream border border-loom-beige rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
        <div>
          <h1 className="font-heading text-3xl md:text-4xl font-bold text-loom-wood">
            बुनकर सहकारी प्रबंधन पटल (Management Center)
          </h1>
          <p className="font-body text-base text-loom-ink-light mt-1">
            समिति प्रबंधक: <span className="font-bold text-loom-wood">{userProfile?.displayName || 'सचिव साहब'}</span> | सहकारी संघ: <span className="font-semibold text-loom-gold">बुनकर सहकारी समिति (coop1)</span>
          </p>
        </div>
      ...
      <div className="vintage-card p-6 mb-8 bg-loom-cream border-t-4 border-t-loom-gold">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            <PlusCircle className="w-5 h-5" />
            बैठक रिकॉर्ड करें (Record Meeting)
          </Link>
          <Link
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                समिति की हालिया गतिविधि (Recent Activity Log)
              </CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                उत्पादन सांख्यिकी (Weaving Statistics)
              </CardTitle>
            </CardHeader>
            <CardContent className="font-body text-sm text-loom-ink-light space-y-4">
              <div className="p-4 bg-loom-cream/50 rounded-xl border border-loom-beige/40">
                <p className="font-heading font-bold text-loom-wood text-base">बनारसी साड़ियाँ (Banarasi Sarees)</p>
                <p className="text-xs text-loom-gold font-bold">मुख्य उत्पाद श्रेणी</p>
              ...
              <div className="p-4 bg-loom-cream/50 rounded-xl border border-loom-beige/40">
                <p className="font-heading font-bold text-loom-wood text-base">इकत थान (Ikat Bundles)</p>
                <p className="text-xs text-loom-gold font-bold">द्वितीयक उत्पाद श्रेणी</p>
                <div className="w-full bg-loom-beige/30 h-2.5 rounded-full mt-2 overflow-hidden">
