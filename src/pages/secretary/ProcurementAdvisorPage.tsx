...
export const ProcurementAdvisorPage: React.FC = () => {
  ...
  const translateReason = (reason: string, item: any) => {
    ...
    if (!isEn) {
      ...
      if (reason.includes("त्योहारी")) {
        return `Stock level (${stockQty} ${unit}) is below reorder level (${reorderLevel} ${unit}). Demand is projected 1.5x higher due to upcoming festival season.`;
      }
      if (reason.includes("आगामी मांग")) {
        return `Stock level (${stockQty} ${unit}) is below reorder level (${reorderLevel} ${unit}). Order immediately to meet upcoming demand.`;
      }
      if (reason.includes("लंबित अनुरोधों")) {
        return `Weavers' pending requests (${pendingQty} ${unit}) exceed the current stock (${stockQty} ${unit}).`;
      }
      if (reason.includes("सुरक्षित स्तर")) {
        return `Stock levels are safe but approaching reorder thresholds. Buffer stock build-up is advised.`;
      }
      if (reason.includes("पर्याप्त मात्रा")) {
        return `Stock is optimal (${stockQty} ${unit}). No immediate procurement is recommended.`;
      }
      if (reason.includes("नई सामग्री")) {
        return `This material is not in stock currently, but weavers have requested a total of ${pendingQty} ${unit}.`;
      }
      return reason;
    }
  };
  ...
  const fetchAdvice = async () => {
    ...
    try {
      ...
      toast.error(isEn ? "Error loading advisor report" : t('advisor.fetchError', 'सलाहकार रिपोर्ट लोड करने में त्रुटि'), { className: 'vintage-toast' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateAutoIndent = async (advice: any) => {
    ...
    try {
      ...
      toast.success(isEn 
        ? `Successfully created draft bulk indent for ${getMaterialTranslation(advice.materialName)}!`
        : t('advisor.indentCreateSuccess', 'सफलतापूर्वक {{material}} के लिए ड्राफ़्ट बल्क इंडेंट बनाया गया!', { material: getMaterialTranslation(advice.materialName) }), { className: 'vintage-toast' });
      ...
      toast.error(isEn ? "Failed to create bulk indent" : t('advisor.indentCreateError', 'इंडेंट बनाने में विफलता'), { className: 'vintage-toast' });
    } finally {
      setCreatingId(null);
    }
  };
  ...
  const currentMonthName = new Date().toLocaleDateString(isEn ? 'en-US' : (i18n.language === 'bn' ? 'bn-IN' : 'hi-IN'), { month: 'long' });
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/40 p-5 rounded-2xl border border-loom-beige/50 backdrop-blur-xs">
          <div>
            <h1 className="font-heading text-3xl font-bold text-loom-wood flex items-center gap-2">
              ...
              {isEn ? "Predictive Procurement Advisor" : t('advisor.title', 'पूर्वानुमान खरीद सलाहकार (Predictive Procurement Advisor)')}
            </h1>
            <p className="font-body text-sm text-loom-ink-light mt-1">
              {isEn 
                ? "Forecast and review raw material requirements according to seasonal demand and inventory analytics."
          ...
          >
            ...
            {isEn ? "Recalculate" : t('advisor.recalculate', 'पुनः गणना करें (Recalculate)')}
          </button>
        </div>
        ...
        <div className="p-4 bg-loom-gold/10 border border-loom-gold/30 rounded-xl flex gap-3 text-loom-ink">
          ...
          <div className="text-sm font-body">
            <span className="font-heading font-bold text-loom-wood block text-base">
              {isEn ? "Smart Insights Active" : t('advisor.insightsActive', 'स्मार्ट विश्लेषण सक्रिय है (Smart Insights Active)')}
            </span>
            {isEn 
              ? `This advisor analyzes seasonal trends and individual weaver requests based on the current month (${currentMonthName}).`
            ...
            {isFestivalSeason ? (
              <span className="text-red-700 font-semibold block mt-1">
                {isEn 
                  ? "⚠️ Currently in festival peak season (Sep-Nov). A 1.5x demand buffer factor has been added."
              ...
              <span className="text-emerald-700 block mt-1">
                {isEn 
                  ? "✓ Market demand is currently stable. Procurement advice conforms to normal reorder levels."
        ...
        {loading ? (
          <div className="py-24 flex flex-col items-center justify-center gap-2">
            ...
            <p className="font-heading text-loom-wood mt-4 text-sm animate-pulse">
              {isEn ? "Analyzing inventories..." : t('advisor.calculating', 'मशीन गणना कर रही है...')}
            </p>
          </div>
        ) : adviceList.length === 0 ? (
          <div className="vintage-card p-12 text-center max-w-2xl mx-auto">
            ...
            <h3 className="font-heading text-2xl font-bold text-loom-wood mb-2">
              {isEn ? "All stock levels are safe" : t('advisor.safeStockTitle', 'सभी स्टॉक सुरक्षित हैं')}
            </h3>
            <p className="font-body text-base text-loom-ink-light">
              {isEn 
                ? "All raw materials for your cooperative society are optimally stocked. No immediate procurement is recommended."
            ...
            {highPriority.length > 0 && (
              <div className="space-y-4">
                <h2 className="font-heading text-xl font-bold text-red-700 flex items-center gap-2 border-b border-red-200 pb-1.5">
                  ...
                  {isEn ? "🔴 High Priority Actions (Urgent Reorders)" : t('advisor.highPriority', '🔴 उच्च प्राथमिकता सिफारिशें (Urgent Actions Required)')}
                </h2>
                    ...
                    >
                      <div>
                        <div className="flex justify-between items-start gap-2 mb-2">
                          ...
                          <span className="text-xs px-2.5 py-1 rounded-full bg-red-50 text-red-700 border border-red-200 font-bold">
                            {isEn ? "Urgent" : t('advisor.urgentBadge', 'अत्यंत आवश्यक')}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2 p-2.5 bg-loom-sand/10 rounded-xl border border-loom-beige/30 font-body text-xs text-center mb-4">
                          <div>
                            <span className="block text-[10px] font-heading text-loom-ink-light">{isEn ? "Current Stock" : t('advisor.currentStock', 'वर्तमान स्टॉक')}</span>
                            <span className="font-bold text-sm text-loom-ink">{item.stockQuantity} {getUnitTranslation(item.unit)}</span>
                          </div>
                          <div>
                            <span className="block text-[10px] font-heading text-loom-ink-light">{isEn ? "Pending Requests" : t('advisor.pendingRequests', 'लंबित अनुरोध')}</span>
                            <span className="font-bold text-sm text-red-600">{item.pendingRequestedQuantity} {getUnitTranslation(item.unit)}</span>
                          </div>
                          <div>
                            <span className="block text-[10px] font-heading text-loom-wood font-bold">{isEn ? "Suggested Order" : t('advisor.suggestedOrder', 'सुझाया गया ऑर्डर')}</span>
                      ...
                      >
                        ...
                        {creatingId === item.materialName 
                          ? (isEn ? "Creating Indent..." : t('advisor.creatingIndent', 'इंडेंट बनाया जा रहा है...')) 
                          : (isEn ? "Auto-Create Draft Indent" : t('advisor.autoCreateBtn', 'ऑटो इंडेंट ड्राफ़्ट बनाएं (Auto-Create Draft)'))}
                      </button>
                    </div>
                  ))}
                </div>
            ...
            {mediumPriority.length > 0 && (
              <div className="space-y-4">
                <h2 className="font-heading text-xl font-bold text-amber-700 flex items-center gap-2 border-b border-amber-200 pb-1.5">
                  ...
                  {isEn ? "🟡 Medium Priority Recommendations (Proactive Orders)" : t('advisor.mediumPriority', '🟡 मध्यम प्राथमिकता सिफारिशें (Proactive Reordering)')}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {mediumPriority.map((item, idx) => (
                    ...
                    >
                      <div>
                        <div className="flex justify-between items-start gap-2 mb-2">
                          <h3 className="font-heading text-lg font-bold text-loom-wood">{getMaterialTranslation(item.materialName)}</h3>
                          <span className="text-xs px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200 font-bold">
                            {isEn ? "Medium" : t('advisor.mediumBadge', 'मध्यम')}
                          </span>
                        </div>
            ...
            {lowPriority.length > 0 && (
              <div className="space-y-4">
                <h2 className="font-heading text-xl font-bold text-emerald-700 flex items-center gap-2 border-b border-emerald-200 pb-1.5">
                  ...
                  {isEn ? "🟢 Optimal Stock Levels (Safe Inventories)" : t('advisor.lowPriority', '🟢 सुरक्षित स्टॉक (Optimal Stock Levels)')}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <div className="flex justify-between items-start gap-2 mb-2">
                          ...
                          <span className="text-xs px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold">
                            {isEn ? "Safe" : t('advisor.safeBadge', 'सुरक्षित')}
                          </span>
                        </div>
